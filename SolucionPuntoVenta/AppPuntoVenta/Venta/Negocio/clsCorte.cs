...
namespace SRATAPV.Ventas.Negocio
{
    class clsCorte
    {
        ...
        public DataSet leerUltimoCorteAbierto()
        {
            BD Objeto = new BD();
            DataSet consulta = new DataSet();
            Objeto.sentenciaSQL = "SELECT cor_keycor FROM proccort WHERE cor_nombre='" + Sesion.Usuario + "' AND cor_estado ='abierto'  ORDER BY cor_keycor desc";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                if (consulta != null && consulta.Tables[0].Rows.Count > 0)
        ...
        public string leerFondoCaja(string idCortes)
        {
            BD Objeto = new BD();
            DataSet consulta = new DataSet();
            Objeto.sentenciaSQL = "SELECT cor_fondocaj FROM proccort WHERE cor_keycor = '" + idCortes + "' and cor_estado = 'Abierto'";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                if (consulta != null && consulta.Tables[0].Rows.Count > 0)
        ...
        public bool ModificarCorte()
        {
            ...
            Objeto.sentenciaSQL = "UPDATE proccort SET  cor_fechFin = GETDATE()," +
                         "cor_totVentas = '" + Convert.ToDecimal(cor_totVentas) + "'," +
                        "cor_totVtasReg = '" + Convert.ToDecimal(cor_totVtasReg) + "'," +
        ...
        public bool GuardarDetalleCorte()//No se usa
        {
            ...
            Objeto.sentenciaSQL = "INSERT INTO [proccort1] "+
                                  "     ([cor1_keycor] "+
                                  "     ,[cor1_moneda] "+
                                  "     ,[cor1_veinte] "+
                                  "     ,[cor1_cincuenta] "+
        ...
        public DataSet leerVentasDelCorteOLD()//No se usa
        {
            BD Objeto = new BD();
            DataSet consulta = new DataSet();
        ...
        public DataSet leerVentasDelCorte()//No se usa
        {
            ...
            Objeto.sentenciaSQL = "SELECT ven_keyven FROM procvent WHERE ven_keycor = '" + _cor_keycor + "' ";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                return consulta;
        ...
        public DataSet leerVentasTotalDelCorte()//No se usa
        {
            BD Objeto = new BD();
            DataSet consulta = new DataSet();

            Objeto.sentenciaSQL = "SELECT * FROM procvent4 WHERE ven4_keyven4 = '" + _cor_keycor + "' ";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                return consulta;
        ...
        public DataSet leerVentasDetalleDelCorte(string idsVen)//No se usa
        {
            BD Objeto = new BD();
            DataSet consulta = new DataSet();

            Objeto.sentenciaSQL = "SELECT ven4_metodopago AS metodopago ,SUM(ven4_importe) AS importe FROM procvent4 WHERE ven4_keyven4 IN ("+idsVen+") GROUP BY ven4_metodopago";
            consulta = Objeto.ejecutaConsulta();
            if (!Objeto.hayError)
            {
                return consulta;
