            ...
            if (busquedaProducto.DialogResult == DialogResult.OK)
            {
                if (!esPromocionRegalo)
                {
                    if (artPromo.Rows.Count > 0)
                    {
                        ...
                        string prePro = "";
                        bool valido = false;
                        //productosValidos.Clear();
                        foreach (DataGridViewRow item in artPromo.Rows)
                        {
                            cod = item.Cells[0].Value.ToString().ToUpper();
                            nomPro = item.Cells[1].Value.ToString().ToUpper();
                            productosValidos.Add(new Producto() { Codigo = cod, NombreArticulo = nomPro});
                        }
                        cod = "";
                        codPro = "";
                        nomPro = "";
                       ...
                       var Existen = (from p in productosValidos
                                   where (from b in productos
                                          select b.Codigo)
                                             .Contains(p.Codigo)
                       ...
                       var result = (from p in productos
                        ...
                        {
                            codPro = item.Codigo.ToString().ToUpper();
                            nomPro = item.NombreArticulo.ToString().ToUpper();
                            productosValidos.Add(new Producto() { Codigo = codPro, NombreArticulo = nomPro });
                        }

                        Console.WriteLine(productosValidos);
                        Console.WriteLine(productos);
                        //productos.Clear();
